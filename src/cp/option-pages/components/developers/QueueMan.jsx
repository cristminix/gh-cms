...
import CourseApi from "@/cp/global/course-api/CourseApi"
import { formatBytes, timeout } from "@/cp/global/fn"
import {
...
import { courseUrlFromSlug } from "@/cp/global/course-api/course_fn"
...
const QueueMan = forwardRef(
  (
    ...
    const [downloadProvider, setDownloadProvider] =
      useState("js-file-downloader")
    ...
    const initQDlStatus = (queueMain, queueData) => {
      ...
      queueMain.items.map((item, idx) => {
        ...
        n_qDlStatusM.push({
          idx,
          state: qState ? qState.mState : QueueState.INIT,
        })
        n_qDlStatusT.push({
          idx,
          state: qState ? qState.tState : QueueState.INIT,
        })
      })
    ...
    const qDlStatusView = (tocId, t = null, defaultText = "") => {
      ...
      if (status) {
        if (t == "m") {
          if (record) {
            if (
              record.mResult === QueueResult.SUCCESS_MEDIA ||
              record.result === QueueResult.SUCCESS
            ) {
          ...
          switch (status) {
            ...
            default:
              ...
              cls =
                (queueRunning || singleMode) && idx == activeQueueIdx
          ...
          if (record) {
            if (
              record.tResult === QueueResult.SUCCESS_TRANS ||
              record.result === QueueResult.SUCCESS
            ) {
        ...
        return (
          <span title={`${QueueState.toStr(status)}:(${status})`}>
            <i className={`fa ${cls}`} />
          </span>
        )
      }
    ...
    const updateQState = async (qItem, qState, state) => {
      ...
      const META_STATES = [
        QueueState.FETCH_META,
        QueueState.FETCH_META_FAIL,
        QueueState.FETCH_META_OK,
        QueueState.FETCH_META_RETRY,
      ]
      const MEDIA_STATES = [
        QueueState.FETCH_MEDIA,
        QueueState.FETCH_MEDIA_FAIL,
        QueueState.FETCH_MEDIA_OK,
        QueueState.FETCH_MEDIA_RETRY,
      ]
      const TRANS_STATES = [
        QueueState.FETCH_TRANS,
        QueueState.FETCH_TRANS_FAIL,
        QueueState.FETCH_TRANS_OK,
        QueueState.FETCH_TRANS_RETRY,
      ]
    ...
    const updateQResult = async (qItem, qState, result) => {
      ...
      if (result == QueueResult.SUCCESS) {
        ...
        result == QueueResult.SUCCESS_MEDIA ||
        result == QueueResult.SUCCESS_TRANS
      ) {
        mode = "warning"
      } else {
        mode = "error"
    ...
    const processQueue_fetchTrans = async (qItem, qState, toc) => {
      ...
      if (transcripts.length == 0) {
        ...
        try {
          const tmpData = await processQueue_fetchMeta(
            qItem,
            qState,
            toc,
            false,
          ...
          console.log(tmpData)
          let [slocs_, transcripts_, err] = tmpData
          if (!err) {
      ...
      if (transcripts.length > 0) {
        ...
        if (vttUrl) {
          try {
            success = await downloadVtt(
              ...
              downloaderRef,
              qState,
              downloadProviderRef.current,
              (e, idx, course, toc, opt, t, provider) =>
                onDownloadProgress(
                  e,
                  idx,
                  course,
        ...
        // await timeout(TEST_ENABLE_TIMEOUT_VALUE)
        // if()
        nQState = await updateQState(
          qItem,
          qState,
          !success ? QueueState.FETCH_TRANS_FAIL : QueueState.FETCH_TRANS_OK,
        )
      }
    ...
    const processQueue_fetchMedia = async (qItem, qState, toc) => {
      ...
      if (slocs.length == 0) {
        nQState = await updateQState(qItem, qState, QueueState.FETCH_META_RETRY)
        let [slocs_, transcripts_, err] = await processQueue_fetchMeta(
          qItem,
          qState,
          toc,
          false,
      ...
      if (slocs.length > 0) {
        ...
        if (mediaUrl) {
          try {
            success = await downloadMedia(
              ...
              qState,
              downloadProviderRef.current,
              (e, idx, course, toc, opt, qState, t, provider) =>
                onDownloadProgress(
                  e,
                  idx,
                  course,
            ...
            if (lastQState) {
              if (lastQState.loaded !== lastQState.size) {
                alert(
                  "Content Size mismatch detected, this caused by unstable net connection",
                )
                success = false
              }
            }
        ...
        nQState = await updateQState(
          qItem,
          qState,
          !success ? QueueState.FETCH_MEDIA_FAIL : QueueState.FETCH_MEDIA_OK,
        )
      }
    ...
    const processQueue_fetchMeta = async (
      qItem,
      qState,
      toc,
      returQState = true,
      ...
      if (returQState) {
        ...
        return [slocs, transcripts, err, nQState]
      }
    }
    const processQueue_result = async (
      mediaQResult,
      transQResult,
      qItem,
      qState,
      ...
      if (
        mediaQResult == QueueResult.SUCCESS_MEDIA &&
        transQResult == QueueResult.SUCCESS_TRANS
      ) {
    ...
    const processQueue_INIT_FAILED = async (qItem, qState, toc) => {
      ...
      if (qState.state == QueueState.FETCH_META_OK) {
        ...
        qState = await processQueue_result(
          mediaQResult,
          transQResult,
          qItem,
          qState,
    ...
    const processQueue = async (qItem) => {
      ...
      if (qItem) {
        ...
        if (toc) {
          ...
          if (!lastQsRec) {
            lastQsRec = await mQState.create(
              course.id,
              toc.id,
              idx,
              QueueState.INIT,
    ...
    const initQDlViews = (queueMain, queueData) => {
      ...
      queueMain.items.map((item, idx) => {
        ...
        n_qDlViewsM.push({
          idx,
          loaded: qState ? qState.mLoaded : 0,
          size: qState ? qState.mSize : 0,
          percentage: mPctg,
        })
        n_qDlViewsT.push({
          idx,
          loaded: qState ? qState.tLoaded : 0,
          size: qState ? qState.tSize : 0,
          percentage: tPctg,
        })
      })
    ...
    const handleQdlViewsMChange = (idx, loaded, size) => {
      ...
      setQDlViewsM((prevQDlViewsM) =>
        prevQDlViewsM.map((n_qDlViewsM) =>
          n_qDlViewsM.idx === idx
            ? { ...n_qDlViewsM, loaded, size, percentage }
            : n_qDlViewsM,
        ),
      )
    }
    const handleQdlViewsTChange = (idx, loaded, size) => {
      ...
      setQDlViewsT((prevQDlViewsT) =>
        prevQDlViewsT.map((n_qDlViewsT) =>
          n_qDlViewsT.idx === idx
            ? { ...n_qDlViewsT, loaded, size, percentage }
            : n_qDlViewsT,
        ),
      )
    }
    ...
    const confirmLeaveWidow = (e) => {
      ...
      var confirmationMessage = ("Are you sure you want to leave?"(
        e || window.event,
      ).returnValue = confirmationMessage) // For some older browsers
    ...
    const startQueueSingle = async (idx, lastResult) => {
      if (queueStartedRef.current) {
        alert(
          "Could not start queue single, Please wait until the last download complete",
        )
        return
      }
    ...
    const checkQueueFinished = async () => {
      if (course) {
        const allFinished = await checkQueueIsAllFinished(
          course.id,
          tocArray,
          mQState,
        )
    ...
    let resetBtnIconCls = !queueResetting
      ? "fa fa-refresh"
      : "fa fa-spin fa-spinner"
    const thCls =
      "px-1 py-1 text-left text-xs font-medium text-gray-500 uppercase"
    ...
    return (
      <>
        <ProxyComponent
          ref={ref}
          proxyCallback={(a, b) => runProxyCallback(a, b)}
        />
                ...
                <div className="flex gap-2">
                  {!singleMode ? (
                    <>
                      {!queueAllFinished ? (
                        <Button
                          ref={startQueueBtnRef}
                          caption={`${queueRunning ? "Queue is running" : "Start Queue"}`}
                      ...
                      {queueAllFinished ? (
                        <Button
                          ref={resetQueueBtnRef}
                          caption={"Reset Queue"}
                          disabled={queueResetting}
                          icon={resetBtnIconCls}
                          onClick={(e) => resetQueue()}
                        />
                      ) : null}
                      {queueRunning ? (
                        <Button
          ...
          {blockMainContent ? (
            ...
            <div className="queue-table border rounded-xl shadow-sm p-6 dark:bg-gray-800 dark:border-gray-700">
              ...
              <div className="flex flex-col">
                <div
                  className={`overflow-x-auto sm:-mx-6 lg:-mx-8 ${niceScrollbarCls}`}
                >
                  <div
                  ...
                  >
                    <div className="overflow-hidden">
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead>
                          <tr>
                            <th className={`${thCls} w-[20px]`} rowSpan={2}>
                              No
                            </th>
                            <th className={`${thCls} w-auto`} rowSpan={2}>
                              Title
                            </th>
                        ...
                        <tbody>
                          {tocArray ? (
                            <>
                              {tocArray.length > 0
                                ? tocArray.map((toc, tidx) => {
                                    const qState = mQState.getRow(
                                      course.id,
                                      toc.id,
                                      tidx,
                                    )
                                            ...
                                            <div className="flex gap-2">
                                              {!queueRunning ? (
                                                <>
                                                  ...
                                                  {DEV_MODE &&
                                                    !QueueResult.FINISHED && (
                                                      <DropdownMenu
                                                        className={`p-0`}
                                                        btnClassName={`px-0 py-0 pl-2 pr-2`}
                                                        data={ddToolbarData}
                                                        onSelect={(e) =>
                                                          onSelectDdToolbar(
                                                            e,
