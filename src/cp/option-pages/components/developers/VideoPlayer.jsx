...
import { isTimeExpired } from "@/cp/global/course-api/course_fn"
import { getName as getCountryName } from "country-list"
// import DropdownMenu from "@/cp/components/shared/ux/DropdownMenu"
import DropdownSelect from "@/cp/components/shared/ux/DropdownSelect"
  ...
  return (
    <div className="video-player py-4 ">
      <div className="flex gap-2 py-2">
        <Button
          onClick={(e) => onSetHideSidebar(e)}
          icon="fa fa-cog"
          caption={`Toggle Sidebar`}
        />
        <DropdownSelect
          selected="Select Fmt"
          data={availableFmtList}
          onSelect={(fmt) => onChangeMediaFmt(fmt)}
        />
      </div>
      ...
      <div className="player-container flex" data-vjs-player>
        ...
        <div
          className={`playlist-container  w-[275px] h-screen flex-none overflow-auto ${niceScrollbarCls}`}
        >
          {playlistData ? (
            <PlaylistControl
              data={playlistData}
              onSelectItem={(a, b) => onSelectItem(a, b)}
            />
          ) : null}
        </div>
      </div>
