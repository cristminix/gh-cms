...
const FetchApi = ({ store, config }) => {
  ...
  return (
    <>
      <Toast ref={toastRef} />
      <DownloadWizard
        getOutFilename={async (a, b, c) => await getOutFilename(a, b, c)}
        toast={(a, b) => toast(a, b)}
        className="mb-2"
        ref={dlWizardRef}
        store={store}
        ...
        course={course}
        toc={toc}
        setMediaOutFilename={(a) => setMediaOutFilename(a)}
        setTranscriptOutFilename={(a) => setTranscriptOutFilename(a)}
      />
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td colSpan={3}>
                        <img className="w-8" src={countryFlagUrl} />
                      </td>
                    </tr>
                    <tr>
                      <th className={`${thCls}`}>Found Matching Trans</th>
                      <td colSpan={3}>{foundMatchingTrans ? <i className=" text-green-500 fa fa-check" /> : <i className=" text-red-500 fa fa-times" />}</td>
                    </tr>
                    <tr>
                      <th className={`${thCls}`}>Found Matching Fmt</th>
                      <td colSpan={3}>{foundMatchingFmt ? <i className=" text-green-500 fa fa-check" /> : <i className=" text-red-500 fa fa-times" />}</td>
                    </tr>
                    {foundMatchingFmt == 0 ? (
                      <tr>
                        <th className={`${thCls}`}>Override Matching Fmt</th>
                        <td colSpan={3}>{overrideMatchingFmt}</td>
                      </tr>
                    ) : null}
                    {foundMatchingTrans == 0 ? (
                      <tr>
                        <th className={`${thCls}`}>Override Matching Trans</th>
                        <td colSpan={3}>{overrideMatchingTrans}</td>
                      </tr>
                    ) : null}
                    <tr>
                      <th className={`${thCls} `}>Media Url</th>
                      <td>
                        <p title={slocUrl} className="w-[600px] mb-1 font-mono text-clip overflow-hidden ...">
                          {slocUrl}
                        </p>
                      </td>
                    ...
                    <tr>
                      <th className={`${thCls}`}>Media Output Filename</th>
                      <td colSpan={3}>{mediaOutFilename}</td>
                    </tr>
                    <tr>
                      <th className={`${thCls} `}>Trans Url</th>
                      <td>
                        <p title={transUrl} className="mb-1 w-[600px] font-mono text-clip overflow-hidden ...">
                          {transUrl}
                        </p>
                      </td>
                      <td></td>
                    ...
                    <tr>
                      <th className={`${thCls}`}>Transcript Output Filename</th>
                      <td colSpan={3}>{transcriptOutFilename}</td>
                    </tr>
