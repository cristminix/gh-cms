...
import { formatBytes } from "@/cp/global/fn"
...
const DBTableManager = ({ store, config }) => {
  ...
  const onReinit = async (item, index) => {
    if (
      confirm(
        `The ${item.table} table is going droped, are you sure want to reinit ?`,
      )
    ) {
    ...
    callbackActions: {
      ...
      edit: (item, index, options, linkCls, gridAction) => {
        return (
          <>
            <Button
              loading={reinitLoading[index]}
              icon="fa fa-trash"
              caption="Truncate"
              onClick={(e) => onReinit(item, index)}
            />
          </>
        )
      },
  ...
  const containerCls =
    "db-table-manager border mb-2 rounded-xl shadow-sm p-6 dark:bg-gray-800 dark:border-gray-700"
  return (
    <div className={containerCls}>
      <div className="explorer-toolbar">
        <div className="flex gap-2">
          <Button
            onClick={(e) => exportDb(e)}
          ...
          <Button
            onClick={(f) => f}
            icon="fa fa-save"
            caption={"Storage Size : " + formatBytes(storageSz)}
          />
        </div>
      </div>
