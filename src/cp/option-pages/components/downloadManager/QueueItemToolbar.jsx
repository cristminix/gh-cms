...
const QueueItemToolbar = ({
  ...
  const icon =
    dlStatusResult == 0 ? "download" : dlStatusResult != 4 ? "refresh" : "check"
  let disabled =
    dlStatusResult == 0
      ? false
  ...
  return (
    <>
      <div className="queue-item-toolbar flex gap-2 items-center">
        {!finished ? (
          <>
            {interupted ? (
              <>
                <Button
                  ...
                  onMouseOver={(e) =>
                    logStatusBar(
                      "QueueItemToolbar.startQueue",
                      `Resume ${toc.title}`,
                    )
                  }
                  onClick={(e) => startQueueItem(vIndex)}
                  caption={""}
                  disabled={disabled}
                  loading={loading}
                  icon={`fa fa-refresh`}
          ...
          <>
            <Button
              ...
              onMouseOver={(e) =>
                logStatusBar(
                  "QueueItemToolbar.resetQueueItem",
                  `Reset ${toc.title}`,
                )
              }
              onClick={(e) => resetQueueItem(vIndex)}
              caption={""}
              disabled={false}
              loading={loading}
              icon={`fa fa-trash`}
        ...
        <DropdownMenu
          labelIcon="fa fa-cog"
          data={ddToolbarData}
          onSelect={(e) => onSelectToolbarItem(e, course, toc)}
          btnClassName="py-[7px] px-0 pr-2"
        />
      </div>
    </>
  )
