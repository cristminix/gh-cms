...
import ProgressBar from "@/cp/components/shared/ux/ProgressBar"
...
async function fetchDownloadReal(
  url,
  outputFilename,
  mime,
  progressCallback,
...
async function fetchDownloadReal_Old(
  url,
  outputFilename,
  mime,
  progressCallback,
  ...
  return new Promise((resolve, reject) => {
    ...
    fetch(url)
      .then((response) => {
        ...
        let contentLength = response.headers.get(
...
async function fetchDownload(
  ...
  if (FETCH_TEST_MODE) {
    ...
    try {
      const data = await fetchDownloadReal(
        url,
        outputFilename,
        mime,
        progressCallback,
...
class QueueMan extends Component {
  ...
  async fetchDlItem(
    ...
    if (url) {
      ...
      this.setState({
        infoMessage: `Download ${outputFilename} ${dlStatus == -1 ? "Failed" : "Success"}`,
      })
  ...
  async fetchDlQueueItem(vIndex, captionUrl, videoUrl, mode = "single") {
    ...
    const { captionStatus, videoStatus, finished, interupted } =
      this.getDmStatus(vIndex)
    ...
    if (mode) {
      ...
      if (!runQueue) {
        let alertMessage =
          mode == "single"
            ? "Cant download this item in single mode because Queue is running"
            : "Cant download this item in queue mode because Queue is not running"
        alert(alertMessage)
        return null
      }
      ...
      if (runQueue) {
        ...
        if (!finished) {
          if (interupted) {
            if (captionStatus != 2) {
              dlCaptionStatus = await this.fetchDlItem(
                vIndex,
                "caption",
                queueItem,
                captionUrl,
              ...
              if (
                confirm(
                  `${captionFilename} already downloaded do you wanna redownload again ${captionFilename}?`,
                )
              ) {
                dlCaptionStatus = await this.fetchDlItem(
                  vIndex,
            ...
            if (videoStatus != 2) {
              dlVideoStatus = await this.fetchDlItem(
                vIndex,
                "video",
                queueItem,
                captionUrl,
              ...
              if (
                confirm(
                  `${videoFilename} already downloaded do you wanna redownload again ${videoFilename}?`,
                )
              ) {
                dlVideoStatus = await this.fetchDlItem(
                  vIndex,
  ...
  async fetchDlMeta(vIndex) {
    ...
    if (dmsetup) {
      const selectedStreamLocs = streamLocations.filter(
        (streamLoc) => streamLoc.fmt == selectedFmt,
      )
  ...
  async startQueueItem(vIndex, mode = "single") {
    ...
    if (!FETCH_TEST_MODE) {
      ...
      if (dmstatus.dlMetaRetryCount <= FETCH_FAILED_MAX_COUNT) {
        ...
        for (
          let retryCount = dmstatus.dlMetaRetryCount;
          retryCount <= FETCH_FAILED_MAX_COUNT;
          retryCount++
        ) {
          if (retryCount > 0) {
          ...
          const [validResource, selectedStreamLoc, selectedTranscript] =
            await this.fetchDlMeta(vIndex)
          ...
          await this.mDMStatus.setDlStatusMeta(
            courseId,
            vIndex,
            status,
            retryCount,
  ...
  async onDlProgress(loaded, total, vIndex, lastReadDate, lastLoaded, t) {
    ...
    if (queueItemRef.current) {
      ...
      if (vIndex) {
        ...
        const speed = formatBytes(
          calculateSpeed(lastLoaded, lastReadDate, new Date()),
        )

        const { captionRef, videoRef } = queueItem.getStatusRef(vIndex)

        if (t == "caption") {
          captionRef.current.setValue(
            `${formatBytes(loaded)} of ${formatBytes(total)}`,
          )
        } else if (t == "video") {
          videoRef.current.setValue(
            `${formatBytes(loaded)} of ${formatBytes(total)}`,
          )
        }
  ...
  async triggerFetchDl() {
    ...
    if (sIndex >= 0 && tIndex >= 0) {
      ...
      this.setState(
        { progressIndex, dlRunning: true, currentIndex: qIndex },
        async () => {
          await this.fetchDl()
          if (queueRunning) {
            this.triggerFetchDl()
  ...
  render() {
    ...
    return (
      <>
        <div className="queueman mb-12 mt-2">
            ...
            {alreadySetup ? (
              <>
                ...
                <QueueTable
                  ...
                  clearStatusBar={clearStatusBar}
                  logStatusBar={logStatusBar}
                  resetQueueItem={(vIndex) => this.resetQueueItem(vIndex)}
                  startQueueItem={(vIndex) => this.startQueueItem(vIndex)}
                  queueItemRef={this.queueItemRef}
                  course={course}
                  sections={sections}
                  tocs={tocs}
                  store={store}
