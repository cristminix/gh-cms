...
import { formatBytes, formatLeadingZeros } from "@/cp/global/fn"
...
class QueueItem extends Component {
  ...
  setDlStatus(t, vIndex, status, retryCount = 0) {
    ...
    const dmstatusRetryKey =
      t == "caption" ? "dlCaptionRetryCount" : "dlVideoRetryCount"
    ...
    if (t == "caption") {
      dlStatusResult =
        parseInt(dmstatus.videoStatus) + parseInt(dmstatus[dmstatusKey])
    } else {
      dlStatusResult =
        parseInt(dmstatus.captionStatus) + parseInt(dmstatus[dmstatusKey])
    }
  ...
  async updateDmStatusRow(courseId, vIndex) {
    ...
    const dmstatus = await this.mDMStatus.updateByCourseId(
      courseId,
      vIndex,
      row,
    )
  ...
  render() {
    ...
    const { captionStatusRefs, videoStatusRefs } = this
    const { loadings, dlvideoStatus, dlcaptionStatus, dmstatusList } =
      this.state
          ...
          <tr key={`${refKey}`} className={trCls}>
            ...
            <td className={tdCls}>
              ...
              <div
                ...
                onMouseOver={(e) =>
                  logStatusBar("QueueItem.toc.title", toc.title)
                }
              >
                {enableFilenameIndex
                  ? `${formatLeadingZeros(number)}-${toc.title}`
                  : toc.title}
              </div>
            </td>
            <td className={tdClsCaptionStatus}>
              {" "}
              <div className="flex">
                <i className="mt-1 fa fa-file-text-o" />{" "}
                <InputDisplay
                  value={captionSz}
                  ref={captionStatusRefs[refKey]}
                />{" "}
                <DLStatus status={dmstatus.captionStatus} />
              </div>
            </td>
            <td className={tdClsVideoStatus}>
              {" "}
              <div className="flex">
                <i className="mt-1 fa fa-file-video-o" />{" "}
                <InputDisplay value={videoSz} ref={videoStatusRefs[refKey]} />{" "}
                <DLStatus status={dmstatus.videoStatus} />
              </div>
            </td>
            <td className={tdCls}>
              {" "}
              <QueueItemToolbar
                ...
                vIndex={vIndex}
                resetQueueItem={resetQueueItem}
                startQueueItem={startQueueItem}
                loading={loadings[refKey]}
                dlStatus={vIndexStatus}
