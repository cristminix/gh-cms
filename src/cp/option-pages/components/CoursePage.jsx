...
import CourseApiLegacy from "@/cp/global/course-api/legacy/CourseApiLegacy"
...
class FetchStateInfo extends Component {
  ...
  render() {
    ...
    return (
      <>
        <div
          className={`flex items-center w-full ${hasError ? "text-red-200" : ""} ${success ? "text-green-200" : ""}`}
        >
          <div className="p-2">
            <i className={icon} />
          </div>
    ...
    if (sections && tocs) {
      ...
      if (selectToc) {
        ...
        if (streamLocations) {
          ...
          toast(
            `Fetch stream locations [${Object.keys(streamLocations).join(",")}]`,
            "success",
          )
        } else {
          toast(
            `Failed to fetch stream locations toc : ${selectToc.title}`,
            "error",
          )
        ...
        if (transcripts) {
          ...
          toast(
            `Fetch transcripts [ ${Object.keys(transcripts).join(", ")} ]`,
            "success",
          )
        } else {
  ...
  const doFetchCourse = async (courseSlug, useM3RecPrxCache = false) => {
    ...
    if (course) {
      ...
      if (authors.length > 0) {
        ...
        toast(
          `Failed to fetch course authors course : ${course.title}`,
          "error",
        )
      }
      ...
      if (sections) {
        ...
        toast(
          `Failed to fetch course sections course : ${course.title}`,
          "error",
        )
      }
  ...
  return (
    <>
      ...
      <FetchStateInfo
        ref={fetchInfoCiRef}
        name="Course Info"
        onFetch={(e) => doFetchCourse(slug, useM3Rec)}
        onRetry={(e) => doFetchCourse(slug, useM3Rec)}
      />
      {course ? (
      ...
      {sections && tocs ? (
        <>
          <FetchStateInfo
            ref={fetchInfoCcRef}
            name="Course Config"
            onFetch={(e) => doFetchDlConfig()}
            onRetry={(e) => doFetchDlConfig()}
          />
