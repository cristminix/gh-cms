...
const ThemeForm = ({
  ...
  const saveForm = async (f) => {
    ...
    const url = apiUrl(["web-theme", pk ? `update/${pk}` : "create"])
  ...
  const getRemoteRowData = async () => {
    const pk = data.id
    const url = apiUrl(["web-theme", pk])

    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        const { previewImage } = data.row
        const previewImageUrl = apiUrl(["web-themes/preview", previewImage])
  ...
  const initFormData = (data) => {
    if (data) {
      ...
      if (isEmpty(previewImage)) {
        ...
        setPreviewImageUrl(apiUrl(["web-themes/preview", previewImage]))
        setPreviewImageValid(true)
      }
      setTimeout(() => {
