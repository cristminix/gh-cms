...
import CompanyForm, { createUntitledCompany } from "./form/CompanyForm"
...
const WebCompany = ({ store, config, pageNumber  }) => {
  ...
  const formId = "basic-modal-web-company"
  ...
  const addForm = async (item, index) => {
    const defaultCompany = createUntitledCompany()
  ...
  const deleteForm = async (item, index) => {
    ...
    if (confirm(`Are you sure want to delete this upload "${item.name}"`)) {
      const url = apiUrl(["web-company/delete", item.id])
      try {
  ...
  const getListState = async (limit = null, page = null) => {
    let response = {}
    const url = apiUrl("web-company/states", { limit, page })
    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        response = data
  ...
  const goToPage = (pageNum) => {
    ...
    document.location.hash = `/builder/web-company/page/${pageNum}`
  ...
  const gridOptions = {
    ...
    widthCls: [""],
    headers: ["id","name","fullName","shortName","address","altAddress","shortAddress","slug","phone","altPhone","mobile","altMobile","email","altEmail","ig","fb","twitter","youtube","website","logo","logoSm","logoMd","logoXl"],
    fields: ["id","name","fullName","shortName","address","altAddress","shortAddress","slug","phone","altPhone","mobile","altMobile","email","altEmail","ig","fb","twitter","youtube","website","logo","logoSm","logoMd","logoXl"],
    enableEdit: true,
    callbackFields: {
    },
    callbackHeaders: {
