...
const WebBlockFeatureManager = ({ store, config, pageNumber ,blockId  }) => {
  ...
  const updateList = async () => {
    ...
    const url = apiUrl("web-block-features", { 
      blockId,       limit, 
  ...
  const deleteForm = async (item, index) => {
    ...
    if (confirm(`Are you sure want to delete this upload "${item.name}"`)) {
      const url = apiUrl(["web-block-feature/delete", item.id])
      try {
  ...
  const getListState = async (limit = null, page = null) => {
    let response = {}
    const url = apiUrl("web-block-feature/states", { limit, page })
    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        response = data
