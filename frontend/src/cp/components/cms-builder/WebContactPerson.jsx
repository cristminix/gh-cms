...
const WebContactPerson = ({ store, config, pageNumber  }) => {
  ...
  const updateList = async () => {
    ...
    const url = apiUrl("web-contact-persons", { 
            limit, 
  ...
  const deleteForm = async (item, index) => {
    ...
    if (confirm(`Are you sure want to delete this upload "${item.name}"`)) {
      const url = apiUrl(["web-contact-person/delete", item.id])
      try {
  ...
  const getListState = async (limit = null, page = null) => {
    let response = {}
    const url = apiUrl("web-contact-person/states", { limit, page })
    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        response = data
  ...
  const gridOptions = {
    ...
    widthCls: [""],
    headers: ["id","siteId","name","shortName","kind","contactDetail","enabled"],
    fields: ["id","siteId","name","shortName","kind","contactDetail","enabled"],
    enableEdit: true,
    callbackFields: {
    },
    callbackHeaders: {
