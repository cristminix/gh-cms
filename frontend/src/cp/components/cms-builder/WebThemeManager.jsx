...
const WebThemeManager = ({ store, config, pageNumber }) => {
  ...
  const deleteForm = async (item, index) => {
    ...
    if (confirm(`Are you sure want to delete this upload "${item.title}"`)) {
      const url = apiUrl(["web-theme/delete", item.id])
      try {
  ...
  const getListState = async (limit = null, page = null) => {
    let response = {}
    const url = apiUrl("web-theme/states", { limit, page })
    try {
  ...
  const gridOptions = {
    ...
    callbackFields: {
      title: (field, value, item, index) => {
        const thumbnailUrl = `${apiUrl(["web-themes", "preview", item.previewImage])}`
        return (
          <>
  ...
  const updateList = async () => {
    ...
    const url = apiUrl("web-themes", { limit, page, order_by, order_dir })
    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        const nGrid = data
