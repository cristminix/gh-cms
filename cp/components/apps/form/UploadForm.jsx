...
const UploadForm = ({
  ...
  const saveForm = async (f) => {
    ...
    const url = cmsApiUrl(["yt-upload", pk ? `update/${pk}` : "create"])
  ...
  const getRemoteRowData = async () => {
    const pk = data.id
    const url = cmsApiUrl(["yt-upload", pk])

    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        const { thumbnail } = data.row
        const thumbnailUrl = cmsApiUrl(["yt-uploads/thumbnails", thumbnail])
  ...
  const initFormData = (data) => {
    if (data) {
      ...
      if (isEmpty(thumbnail)) {
        ...
        setThumbnailUrl(cmsApiUrl(["yt-uploads/thumbnails", thumbnail]))
        setThumbnailValid(true)
      }
      setTimeout(() => {
