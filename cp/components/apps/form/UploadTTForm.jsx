...
const UploadTTForm = ({
  ...
  const saveForm = async (f) => {
    ...
    const url = cmsApiUrl(["yt-upload-tt", pk ? `update/${pk}` : "create"])
  ...
  const getRemoteRowData = async () => {
    const pk = data.id
    const url = cmsApiUrl(["yt-upload-tt", pk])

    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        const { thumbnail } = data.row
        const thumbnailUrl = cmsApiUrl(["yt-upload-tts/thumbnails", thumbnail])
  ...
  const initFormData = (data) => {
    if (data) {
      ...
      if (isEmpty(thumbnail)) {
        ...
        setThumbnailUrl(cmsApiUrl(["yt-upload-tts/thumbnails", thumbnail]))
        setThumbnailValid(true)
      }
      setTimeout(() => {
