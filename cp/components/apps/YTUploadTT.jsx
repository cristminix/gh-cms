...
const YTUploadTT = ({ store, config, pageNumber, uploadId }) => {
  ...
  const compareForm = async () => {
    alert(`Coming Soon`)
  }
  const previewForm = async (item, index) => {
    alert(`Coming Soon`)
  }
  const deleteForm = async (item, index) => {
    // console.log(item)
    if (confirm(`Are you sure want to delete this upload "${item.title}"`)) {
      const url = cmsApiUrl(["yt-upload-tt/delete", item.id])
  ...
  const cloneForm = async (item, index) => {
    // console.log(item)
    let kind = "upload"
    let pk = uploadId
    if (item) {
    ...
    const messageClone =
      kind === "upload"
        ? `source parent "${uploadId}"`
        : `selected item : ${item.title}`
    if (confirm(`Are you sure want to clone from ${messageClone}`)) {
      const url = cmsApiUrl(["yt-upload-tt/clone"])
      ...
      try {
        ...
        if (validJson) {
          ...
          toast(
            `Failed to clone id:${uploadId} server sent http ${code} ${text}`,
            "error",
          )
        }
      } catch (e) {
  ...
  const getListState = async (limit = null, page = null) => {
    let response = {}
    const url = cmsApiUrl(["yt-upload-tt/states", uploadId], { limit, page })
    try {
  ...
  const backToYtUpload = () => {
    document.location.hash = `/apps/yt-upload/page/${lastParentPage}`
  }
  const goToPage = (pageNum) => {
    pageNum = parseInt(pageNum) || 1
    document.location.hash = `/apps/yt-upload-tt/${uploadId}/page/${pageNum}?parentPage=${lastParentPage}`
  ...
  const gridOptions = {
    ...
    callbackFields: {
      title: (field, value, item, index) => {
        const thumbnailUrl = `${cmsApiUrl(["yt-upload-tts", "thumbnails", item.thumbnail])}`
        ...
        return (
          <>
            <Button
              ...
              icon="fa fa-eye"
              caption=""
              onClick={(e) => previewForm(item, index)}
            />
            <Button
              title="Clone"
              loading={false}
              icon="fa fa-copy"
              caption=""
              onClick={(e) => cloneForm(item, index)}
            />
            <Button
              title="Edit"
              loading={false}
  ...
  const updateList = async () => {
    ...
    const url = cmsApiUrl("yt-upload-tts", {
      uploadId,
      limit,
      page,
      order_by,
      ...
      <div className={`user-manager ${containerCls}`}>
              ...
              <>
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    onClick={(e) => cloneForm()}
                    icon="fa fa-copy"
                    caption="CLone Source"
                  />
                  <Button
                    onClick={(e) => compareForm()}
                    icon="fa fa-filter"
                    caption="Compare"
                  />
                </div>
        ...
        <div className="flex flex-col ">
          ...
          <div className="pager-container mt-3">
            {grid ? (
              <Pager
                path={`/apps/yt-upload-tt/${uploadId}`}
                page={grid.page}
