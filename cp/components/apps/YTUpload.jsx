...
const YTUpload = ({ store, config, pageNumber }) => {
  ...
  const deleteForm = async (item, index) => {
    ...
    if (confirm(`Are you sure want to delete this upload "${item.title}"`)) {
      const url = cmsApiUrl(["yt-upload/delete", item.id])
      try {
  ...
  const goToTT = (item) => {
    document.location.hash = `/apps/yt-upload-tt/${item.id}?parentPage=${grid.page}`
  }
  const getListState = async (limit = null, page = null) => {
    let response = {}
    const url = cmsApiUrl("yt-upload/states", { limit, page })
    try {
  ...
  const goToPage = (pageNum) => {
    ...
    document.location.hash = `/apps/yt-upload/page/${pageNum}`
  ...
  const gridOptions = {
    ...
    callbackFields: {
      title: (field, value, item, index) => {
        const thumbnailUrl = `${cmsApiUrl(["yt-uploads", "thumbnails", item.thumbnail])}`
        return (
          <>
        ...
        return (
          <>
            <Button
              ...
              caption={`${item.ttCount}`}
              onClick={(e) => goToTT(item, index)}
            />
            <Button
              title="Edit"
              loading={false}
  ...
  const updateList = async () => {
    ...
    const url = cmsApiUrl("yt-uploads", { limit, page, order_by, order_dir })
    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        const nGrid = data
