...
const BlockForm = ({
  ...
  const saveForm = async (f) => {
    ...
    const url = cmsApiUrl(["web-block", pk ? `update/${pk}` : "create"])
  ...
  const getRemoteRowData = async (pk_) => {
    const url = cmsApiUrl(["web-block", pk_])

    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        // const { previewImage } = data.data
        // const previewImageUrl = cmsApiUrl(["web-block/previews", previewImage])
  ...
  const setFormData = (data) => {
    ...
    if (isEmpty(previewImage)) {
      ...
      setPreviewImageUrl(cmsApiUrl(["web-block/previews", previewImage]))
      setPreviewImageValid(true)
    }
  }
