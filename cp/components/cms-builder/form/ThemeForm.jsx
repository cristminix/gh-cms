...
const createUntitledUpload = () => {
  ...
  return { name, slug, description, previewImage }
}
...
const ThemeForm = ({
  ...
  const calculateFormChecksum = (data = null) => {
    ...
    if (data) {
      const { id, name, description, slug, previewImage } = data
      formDataItem = {
        id,
        name,
        description,
  ...
  const saveForm = async (f) => {
    ...
    const formDataItem = { id: pk, name, description, slug }
    ...
    const url = cmsApiUrl(["web-theme", pk ? `update/${pk}` : "create"])
  ...
  const getRemoteRowData = async () => {
    const pk = data.id
    const url = cmsApiUrl(["web-theme", pk])

    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        const { previewImage } = data.row
        const previewImageUrl = cmsApiUrl(["web-themes/preview", previewImage])
  ...
  const initFormData = (data) => {
    if (data) {
      ...
      if (isEmpty(previewImage)) {
        ...
        setPreviewImageUrl(cmsApiUrl(["web-themes/preview", previewImage]))
        setPreviewImageValid(true)
      }
      setTimeout(() => {
