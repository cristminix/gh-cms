...
import SiteSettingForm, { createUntitledSiteSetting } from "./form/SiteSettingForm"
...
const WebSiteSetting = ({ store, config, pageNumber }) => {
  ...
  const formId = "basic-modal-web-site-setting"
  ...
  const updateList = async () => {
    ...
    const url = cmsApiUrl("web-site-settings", {
      limit,
      page,
      order_by,
      order_dir,
  ...
  const addForm = async (item, index) => {
    ...
    setFormData(defaultSiteSetting)
    setShowForm(true)
  ...
  const deleteForm = async (item, index) => {
    ...
    if (confirm(`Are you sure want to delete this upload "${item.name}"`)) {
      const url = cmsApiUrl(["web-site-setting/delete", item.id])
      try {
  ...
  const getListState = async (limit = null, page = null) => {
    let response = {}
    const url = cmsApiUrl("web-site-setting/states", { limit, page })
    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        response = data
  ...
  const goToPage = (pageNum) => {
    ...
    document.location.hash = `/builder/website-setting/page/${pageNum}`
