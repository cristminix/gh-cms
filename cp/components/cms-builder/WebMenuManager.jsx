...
import { Prx, getInputFileContent, requestIdentityToken } from "@cp/global/fn"

import MenuForm, { createUntitledMenu } from "./web-menu-manager/MenuForm"
...
const WebMenuManager = ({ config, store }) => {
  ...
  const formId = "basic-modal-web-menu"
  ...
  const saveMenu = async (e) => {
    ...
    // const url = cmsApiUrl("config/saveMenu")
  ...
  const updateList = async () => {
    ...
    const limit = 100,
      order_by = "id",
      order_dir = "asc"
    const url = cmsApiUrl("web-menus", { limit, page, order_by, order_dir })
    try {
      ...
      if (validJson) {
        ...
        // })
        const { records } = data
        const menus = Menu.arrayToTreeData(records)
              ...
              {treeMenuData.hasData ? (
                <TreeTable config={config} value={treeMenuData} height={vH} onChange={handleOnChange2}>
                  ...
                  <TreeTable.Column
                    ...
                    grow="1"
                    renderCell={(row) => {
                      return <span className="link-cell font-mono">{row.data.link}</span>
                    }}
                    renderHeaderCell={() => <code>Link</code>}
                  />
                  <TreeTable.Column
                    basis="80px"
                    grow="0"
                    renderCell={(row) => {
                      return <span className="target-cell font-mono">{row.data.target}</span>
                    }}
                    renderHeaderCell={() => <code>target</code>}
                  />
                  <TreeTable.Column
                    basis="80px"
                    grow="0"
