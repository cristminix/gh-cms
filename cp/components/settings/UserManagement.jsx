...
import UserForm, { createUntitledUser } from "./form/UserForm"
...
const UserManagement = ({ store, config, pageNumber }) => {
  ...
  const formId = "basic-modal-cms-user"
  ...
  const addForm = async (item, index) => {
    const defaultUser = createUntitledUser()
  ...
  const deleteForm = async (item, index) => {
    // console.log(item)
    if (confirm(`Are you sure want to delete this upload "${item.username}"`)) {
      const url = cmsApiUrl(["cms-user/delete", item.id])
      try {
  ...
  const getListState = async (limit = null, page = null) => {
    let response = {}
    const url = cmsApiUrl("cms-user/states", { limit, page })
    try {
      const { data, validJson, code, text } = await Prx.get(url, requestToken)
      if (validJson) {
        response = data
  ...
  const goToPage = (pageNum) => {
    ...
    document.location.hash = `/settings/user-manager/page/${pageNum}`
  ...
  const gridOptions = {
    ...
    widthCls: [""],
    headers: ["User"], //["id","username","passwd","email","firstName","lastName","displayName","avatarUrl","groupId","createdBy","createDate","lastUpdated"],
    fields: ["username"], //["id","username","passwd","email","firstName","lastName","displayName","avatarUrl","groupId","createdBy","createDate","lastUpdated"],
    enableEdit: true,
    callbackFields: {
      username: (field, value, item, index) => {
        return (
          <div>
            <h2 className="font-bold">
              {item.displayName} (<span className="p-y italic">{item.groupName}</span>)
            </h2>
            <p>
              <span className="pr-1 italic font-bold">{item.username}</span>
              <span className="pr-1 underline">{item.email}</span>
            </p>
          </div>
        )
      },
