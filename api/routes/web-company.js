...
class WebCompanyRouter extends AuthenticatedRouter {
  ...
  async create(req, res) {
    ...
    try {
      const webcompany = await this.mWebCompany.create(
        name,
        address,
        shortAddress,
        slug,
  ...
  async getDropdown(req, res) {
    ...
    const results = await this.mWebCompany.getList(1, 100, "id", "asc")
  ...
  initRouter() {
    this.router.get("/web-company/dropdown/:key?", async (req, res) => await this.getDropdown(req, res))
