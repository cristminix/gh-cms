...
export class MWebContactPerson {
  ...
  async create(siteId, name, kind, contactDetail, enabled) {
    ...
    webcontactperson.enabled = enabled
  ...
  async getList(page = 1, limit = 5, order_by = "id", order_dir = "asc", filter = null) {
    ...
    try {
      ...
      const webcontactpersons = await this.manager.find(WebContactPerson, option)
