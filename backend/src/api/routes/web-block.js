...
class WebBlockRouter extends AuthenticatedRouter {
  ...
  async getState(req, res) {
    ...
    page = parseInt(page) || null;
    const results = await this.mWebBlock.getState(
      templateId,
      limit,
      page,
      kind,
  ...
  async getList(req, res) {
    ...
    const results = await this.mWebBlock.getList(
      templateId,
      page,
      limit,
      order_by,
  ...
  async create(req, res) {
    ...
    try {
      ...
      const webblock = await this.mWebBlock.create(
        templateId,
        name,
        description,
        slug,
      ...
      if (webblock) {
        if (kind === "section") {
          templateBlock = await this.mWebTemplateBlock.create(
            templateId,
            webblock.id
          );
        }
        if (kind === "block" && parent != null) {
          sectionBlock = await this.mWebSectionBlock.create(
            parent,
            webblock.id
          );
        }
  ...
  async delete(req, res) {
    const { templateId, parent } = req.body;
    let id = null;
    if (req.body.id) {
      id = req.body.id;
    }
  ...
  initRouter() {
    ...
    this.router.post(
      ...
      check("templateId", "templateId is required")
        .not()
        .isEmpty(),
