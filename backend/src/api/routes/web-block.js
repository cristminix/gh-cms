...
class WebBlockRouter extends AuthenticatedRouter {
    ...
    async create(req,res){
        ...
        let { templateId, name, slug, kind, path }= req.body
        try {
            const  webblock = await this.mWebBlock.create(templateId, name, slug, kind, path,previewImage)
            return res.send({ row: webblock })
        }catch(e){
     ...
     async update(req,res){
        ...
        if (existingRec) {
            ...
            const { templateId, name, slug, kind, path } = req.body
            const updatedData = { templateId, name, slug, kind, path }
            if(fileUpdated){
    ...
    initRouter(){
        ...
        this.router.put('/web-block/update/:id?',
            (req, res, next) => this.authenticateToken(req, res, next),
        ...
        this.router.delete('/web-block/delete/:id?',
            (req, res, next) => this.authenticateToken(req, res, next),
