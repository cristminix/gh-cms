...
class WebBlockFeatureRouter extends AuthenticatedRouter {
    ...
    async getState(req, res) {
        ...
        const results = await this.mWebBlockFeature.getState(limit, page)
        return res.send(results)
    }
    async getList(req, res){
        const {page,limit,order_by,order_dir} = req.query
        const results = await this.mWebBlockFeature.getList(page,limit,order_by,order_dir)
        return res.send(results)
    }    
    ...
    initRouter(){
        ...
        this.router.get('/web-block-features', 
            (req, res, next) => this.authenticateToken(req, res, next),
            (req, res) => this.getList(req, res))

        this.router.get("/web-block-feature/states",
                (req, res, next) => this.authenticateToken(req, res, next),
                (req, res) => this.getState(req, res))
        
        this.router.get('/web-block-feature/:id', 
            (req, res, next) => this.authenticateToken(req, res, next),
        
            (req, res) => this.get(req, res))
          
        this.router.post('/web-block-feature/create',
            (req, res, next) => this.authenticateToken(req, res, next),
        ...
        this.router.put('/web-block-feature/update/:id?',
            (req, res, next) => this.authenticateToken(req, res, next),
        ...
        this.router.delete('/web-block-feature/delete/:id?',
            (req, res, next) => this.authenticateToken(req, res, next),
